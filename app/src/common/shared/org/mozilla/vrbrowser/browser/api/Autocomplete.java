 ...
 * <code>return WPEResult.fromValue(prompt.dismiss())</code>
 * which terminates this saving request, or confirm it via
 * <code>return WPEResult.fromValue(prompt.confirm(login))</code>
...
public class Autocomplete {
    ...
    public static class CreditCard {
        ...
        @AnyThread
        /* package */ @NonNull Bundle toBundle() {
    ...
    public interface StorageDelegate {
         ...
         * @return A {@link ResultAPI} that completes with an array of
         *         {@link LoginEntry} containing the existing logins for the
         *         given domain.
         */
        @UiThread
        default @Nullable
        ResultAPI<LoginEntry[]> onLoginFetch(
        ...
        @UiThread
        default @Nullable
        ResultAPI<CreditCard[]> onCreditCardFetch() {
            return null;
        }
    ...
    /* package */ final static class StorageProxy { //implements BundleEventListener {
        ...
        public synchronized void handleMessage(
            ...
            if (FETCH_LOGIN_EVENT.equals(event)) {
                ...
                final ResultAPI<LoginEntry[]> result =
                        mDelegate.onLoginFetch(domain);

                if (result == null) {
                    callback.sendSuccess(new Bundle[0]);
                    return;
                }
                ...
                callback.resolveTo(result.map(logins -> {
                    ...
                    final Bundle[] loginBundles =
                            new Bundle[logins.length];
                ...
                final ResultAPI<CreditCard[]> result =
                ...
                callback.resolveTo(result.map(creditCards -> {
                    ...
                    // This is a one-liner with streams (API level 24).
                    final Bundle[] creditCardBundles =
                            new Bundle[creditCards.length];
                ...
                final Bundle loginBundle = message.getBundle("login");
