 ...
 */
public final class BundleAPI implements Parcelable {
    private static final String LOGTAG = "WPEBundle";
    ...
    private static final BundleAPI[] EMPTY_BUNDLE_ARRAY = new BundleAPI[0];

    private SimpleArrayMap<String, Object> mMap;

    /**
     * Construct an empty WPEBundle.
     */
    public BundleAPI() {
    ...
    /**
     * Construct an empty WPEBundle with specific capacity.
     *
     * @param capacity Initial capacity.
     */
    public BundleAPI(final int capacity) {
        mMap = new SimpleArrayMap<>(capacity);
    }

    /**
     * Construct a copy of another WPEBundle.
     *
     * @param bundle WPEBundle to copy from.
     */
    public BundleAPI(final BundleAPI bundle) {
        mMap = new SimpleArrayMap<>(bundle.mMap);
    }

    private BundleAPI(final String[] keys, final Object[] values) {
    ...
    // The only case where we convert String[] to/from WPEBundle[] is if every element
    // is null.
    ...
    /**
     * Returns the value associated with a WPEBundle mapping, or null if the mapping
     * does not exist.
     *
     * @param key Key to look for.
     * @return WPEBundle value
     */
    public BundleAPI getBundle(final String key) {
        return (BundleAPI) mMap.get(key);
    }

    /**
     * Returns the value associated with a WPEBundle array mapping, or null if the
     * mapping does not exist.
     *
     * @param key Key to look for.
     * @return WPEBundle array value
     */
    public BundleAPI[] getBundleArray(final String key) {
        final Object value = mMap.get(key);
        return value == null ? null : Array.getLength(value) == 0 ? EMPTY_BUNDLE_ARRAY :
                !(value instanceof BundleAPI[]) ? new BundleAPI[getNullArrayLength(value)] :
                        (BundleAPI[]) value;
    }

    /**
     * Returns whether this WPEBundle has no mappings.
     *
    ...
    /**
     * Map a key to a WPEBundle value.
     *
     * @param key Key to map.
     * @param value Value to map to.
     */
    public void putBundle(final String key, final BundleAPI value) {
        mMap.put(key, value);
    }

    /**
     * Map a key to a WPEBundle array value.
     *
     * @param key Key to map.
     * @param value Value to map to.
     */
    public void putBundleArray(final String key, final BundleAPI[] value) {
        mMap.put(key, value);
    }
     ...
     * @param value Value to map to.
     */
    public void putBundleArray(final String key, final Collection<BundleAPI> value) {
        if (value == null) {
            mMap.put(key, null);
            return;
        }
        final BundleAPI[] array = new BundleAPI[value.size()];
        int i = 0;
        for (final BundleAPI element : value) {
            array[i++] = element;
        }
        mMap.put(key, array);
    }
    ...
    /**
     * Returns number of mappings in this WPEBundle.
     *
    ...
    public boolean equals(final Object other) {
        if (!(other instanceof BundleAPI)) {
            return false;
        }

        // Support library's SimpleArrayMap.equals is buggy, so roll our own version.
        final SimpleArrayMap<String, Object> otherMap = ((BundleAPI) other).mMap;
        if (mMap == otherMap) {
            return true;
        }
    ...
    public JSONObject toJSONObject() throws JSONException {
        ...
        for (int i = 0; i < mMap.size(); i++) {
            ...
            if (value instanceof BundleAPI) {
                jsonValue = ((BundleAPI) value).toJSONObject();
            } else if (value instanceof BundleAPI[]) {
                final BundleAPI[] array = (BundleAPI[]) value;
                final JSONArray jsonArray = new JSONArray();
                for (final BundleAPI element : array) {
    ...
    public Bundle toBundle() {
        ...
        for (int i = 0; i < mMap.size(); i++) {
            ...
            } else if (val instanceof BundleAPI) {
                out.putBundle(key, ((BundleAPI) val).toBundle());
            } else if (val instanceof BundleAPI[]) {
                final BundleAPI[] array = (BundleAPI[]) val;
    ...
    public static BundleAPI fromBundle(final Bundle bundle) {
        if (bundle == null) {
            return null;
        }
        ...
        for (final String key : bundle.keySet()) {
            ...
            if (value instanceof Bundle || value == null) {
                ...
                final BundleAPI[] out = new BundleAPI[array.length];
                for (int j = 0; j < array.length; j++) {
        ...
        return new BundleAPI(keys, values);
    }
    ...
    public static BundleAPI fromJSONObject(final JSONObject obj) throws JSONException {
    ...
    public void readFromParcel(final Parcel source) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (val instanceof Parcelable[]) {
                ...
                val = Arrays.copyOf(array, array.length, BundleAPI[].class);
            }
    ...
    public static final Parcelable.Creator<BundleAPI> CREATOR =
            new Parcelable.Creator<BundleAPI>() {
                @Override
                public BundleAPI createFromParcel(final Parcel source) {
                    final BundleAPI bundle = new BundleAPI(0);
                    bundle.readFromParcel(source);
                    return bundle;
                }

                @Override
                public BundleAPI[] newArray(final int size) {
                    return new BundleAPI[size];
                }
            };
}
