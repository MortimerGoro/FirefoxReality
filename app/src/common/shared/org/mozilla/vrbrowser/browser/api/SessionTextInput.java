...
/**
 * {@code SessionTextInput} handles text input for {@code SessionAPI} through key events or input
 ...
 * #onCreateInputConnection} and methods in {@link SessionAPI.TextInputDelegate} for changes in
 * behavior in this viewless mode.
 */
public final class SessionTextInput {
    /* package */ static final String LOGTAG = "SessionAPITextInput";
    ...
    /* package */ interface EditableListener {
        ...
        final int NOTIFY_IME_OF_TOKEN = -3;
        ...
        final int IME_FLAG_PRIVATE_BROWSING = 1;
        final int IME_FLAG_USER_ACTION = 2;
    ...
    private static final class DefaultDelegate implements TextInputDelegate {
        ...
        @Override
        public void restartInput(@NonNull final SessionAPI session, final int reason) {
        ...
        @Override
        public void showSoftInput(@NonNull final SessionAPI session) {
            //ThreadUtils.assertOnUiThread();
            final View view = session.getTextInput().getView();
            final InputMethodManager imm = getInputMethodManager(view);
            if (imm != null) {
        ...
        @Override
        public void hideSoftInput(@NonNull final SessionAPI session) {
            //ThreadUtils.assertOnUiThread();
            final View view = session.getTextInput().getView();
            final InputMethodManager imm = getInputMethodManager(view);
            if (imm != null) {
        ...
        @Override
        public void updateSelection(@NonNull final SessionAPI session,
        ...
        @Override
        public void updateExtractedText(@NonNull final SessionAPI session,
        ...
        @TargetApi(21)
        @Override
        public void updateCursorAnchorInfo(@NonNull final SessionAPI session,
    ...
    private TextInputDelegate mDelegate;

    public SessionTextInput(final @NonNull SessionAPI session) { //final @NonNull NativeQueue queue) {
        mSession = session;
        //mQueue = queue;
        //mEditable = new GeckoEditable(session);
    }

    /* void onWindowChanged(final SessionAPI.Window window) {
        if (mQueue.isReady()) {
     ...
     */
    @UiThread
    public void setDelegate(@Nullable final TextInputDelegate delegate) {
        //ThreadUtils.assertOnUiThread();
        mDelegate = delegate;
    }
     ...
     */
    @UiThread
    public @NonNull TextInputDelegate getDelegate() {
        //ThreadUtils.assertOnUiThread();
        //if (mDelegate == null) {
